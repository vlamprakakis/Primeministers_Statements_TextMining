...
text2 <- readLines("tsipras.txt", encoding = defaultEncoding)
...
b <- ggplot(subset(wf, freq>9), aes(x = reorder(word, -freq), y = freq)) +
...
findAssocs(tdm, c("θα"), corlimit = 0.32)

# find correlation of term "κυβέρνηση" with other terms
findAssocs(tdm, c("κυβέρνηση"), corlimit = 0.5)

# find correlation of term "λαού" with other terms
findAssocs(tdm, c("λαού"), corlimit = 0.39)
...
# find correlation of term "μάχη" with other terms
findAssocs(tdm, c("μάχη"), corlimit = 0.5)
...
txt <- readLines ("tsipras.txt", encoding = defaultEncoding)
