...
text2 <- readLines("papandreou.txt", encoding = defaultEncoding)
...
# remove (extra) stopwords -- no need for tsipras
#  -- docs <- tm_map(docs,removeWords, c("νέας", "δημοκρατίας", "χειροκροτήματα", "παρατεταμένα", "ζωηρά", "πτέρυγα"))
...
findAssocs(tdm, c("θα"), corlimit = 0.25)

# find correlation of term "ελλάδα" with other terms
findAssocs(tdm, c("ελλάδα"), corlimit = 0.4)

# find correlation of term "απαξίωση" with other terms
findAssocs(tdm, c("απαξίωση"), corlimit = 0.5)

# find correlation of term "δημόσιο" with other terms
findAssocs(tdm, c("δημόσιο"), corlimit = 0.35)

# find correlation of term "ανάπτυξη" with other terms
findAssocs(tdm, c("ανάπτυξη"), corlimit = 0.4)
...
txt <- readLines ("papandreou.txt", encoding = defaultEncoding)
